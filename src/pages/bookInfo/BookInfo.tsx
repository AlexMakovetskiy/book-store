...
const BookInfo = () => {
	...
	useEffect(() => {
		...
		dispatch(getBookInfo(params.id ?? ""));
	}, [dispatch, params.id]);
	...
	function goToTwitter() {
		...
		return window.open(
			`${BASE_TWITTER_SEARCH + namesBookAuthors[0]}%20${namesBookAuthors[1]}${ENDPOINT_TWITTER_SEARCH}`,
			"_blank",
		);
	}
		...
		<div className="bookinfo-wrapper">
				...
				<div className="specification-wrapper">
					<div className="specification-wrapper__price-container">
						...
						<div className="specification-wrapper__price-container__rating">
							<img
								src="/assets/vector/pages/bookinfo/rating.svg"
								alt="rating"
								className="specification-wrapper__price-container__rating__stars"
							/>
						</div>
					</div>
					...
					<button className="specification-wrapper__preview-book-cover" onClick={showPreview}>
						Preview book
					</button>
				</div>
			</div>
			...
			<div className="contacts">
				<button className="facebook-link" onClick={goToFacebook}>
					<img
						src="/assets/vector/pages/bookinfo/Contacts/facebookLogo.svg"
						alt="facebook"
						className="contacts__social-media"
					/>
				</button>
				<button className="twitter-link" onClick={goToTwitter}>
					<img
						src="/assets/vector/pages/bookinfo/Contacts/twitterLogo.svg"
						alt="twitter"
						className="contacts__social-media"
					/>
				</button>
				<img
					src="/assets/vector/pages/bookinfo/Contacts/otherContactImg.svg"
					alt="other"
					className="contacts__social-media"
				/>
			</div>
			<Subscription />
			{isOpenPreview && <PreviewBookCover handleClose={showPreview} />}
		</div>
	);
};
