...
import {
	...
	deceaseBasketSum,
	deleteBasketBook,
} from "../../services/redux/features/basketBooks/BasketBooksSlice";
...
const Basket = () => {
	...
	const calculatedBookPrice = basketBooks.reduce(
		(acc: number, curr: { count: number; price: number | string }) =>
			acc + curr.count * Number(String(curr.price).replace("$", "")),
		0,
	);
		...
		return basketBooks.map((basketBook: IBasketBook, index) => (
			<div className="basket-book-wrap" key={index}>
				...
				<div className="book-info-container">
					...
					<p className="book-info-container__subtitle">
						by {basketBook.authors}, {basketBook.publisher}
					</p>
					<div className="set-price-container">
						<button
							className="set-price-container__decrease-price"
							onClick={() => dispatch(deceaseBasketSum(basketBook.isbn13))}>
							<img
								src="/assets/vector/pages/basket/minus-logo.svg"
								alt="decrease price"
								className="set-price-container__decrease-price__logo"
							/>
						</button>
						<span className="set-price-container__title custom-font">{basketBook.count}</span>
						<button
							className="set-price-container__encrease-price"
							onClick={() => dispatch(increaseBasketSum(basketBook.isbn13))}>
							<img
								src="/assets/vector/pages/basket/plus-logo.svg"
								alt="encrease price"
								className="set-price-container__encrease-price__logo"
							/>
						</button>
					</div>
				</div>
				...
				<button className="basket-book-wrap__remove-book" onClick={() => dispatch(deleteBasketBook(basketBook.isbn13))}>
					<img
						src="/assets/vector/components/header/search-button/cancelSearch.svg"
						alt="delete book"
						className="basket-book-wrap__remove-book__logo"></img>
				</button>
			</div>
		));
	};
